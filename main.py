...
def mask_database_url(url: Optional[str]) -> str: 
    ...
    try: 
        ...
        if p.scheme in ("sqlite",) and p.path: 
            path = p.path 
            if len(path) > 20: return f"{p.scheme}://{path[:20]}..." 
...
def csv_row_generator(rows: Iterator[models.NotarialSession]) -> Iterator[str]: 
    ...
    writer.writerow([ 
        "Transaction Date", "Signer Name(s)", "Location / Medium", "Platform (RON)", 
        "Document Title", "Act Type", "Statutory Fee", "Additional Fee", "Payment Status", "Seal Reference ID" 
    ]) 
    ...
    for s in rows: 
        try: 
            ...
            for act in acts: 
                if act is None: 
                    document_title, act_type, statutory_fee, additional_fee = "", "", "", "" 
                else: 
                ...
                row = [ 
                    date_str, signer_str, getattr(s, "location_type", "") or "", getattr(s, "ron_platform", "") or "", 
                    document_title, act_type, statutory_fee, additional_fee, 
...
if __name__ == "__main__": 
    ...
    # Notice we pass the app instance name as a string pointing to main:app
